...
DEFAULT_WEB_APP_VERSION = "20260409_6"
...
def encode_urlsafe_json(value: object) -> str:
    raw = json.dumps(value, ensure_ascii=False).encode("utf-8")
    encoded = base64.urlsafe_b64encode(raw).decode("ascii")
    return encoded.rstrip("=")


def build_web_app_url_for_user() -> str:
    categories = load_categories()
    products = load_products()
    cats_token = encode_urlsafe_json(categories)
    products_token = encode_urlsafe_json(products)
    separator = "&" if "?" in WEB_APP_URL else "?"
    return f"{WEB_APP_URL}{separator}cats={cats_token}&items={products_token}"
...
def get_main_keyboard():
    ...
    builder.row(KeyboardButton(
        ...
        web_app=WebAppInfo(url=build_web_app_url_for_user())
    ))
...
async def command_start_handler(message: types.Message) -> None:
    ...
    await message.answer(
        ...
        f"Если ты менял категории или товары, отправь /open для обновленной кнопки.",
