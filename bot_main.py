...
def load_dotenv_file(path: Path = Path(".env")) -> None:
    ...
    try:
        ...
        # If .env is malformed, continue with regular environment variables.
        pass
...
# Дополнительно поддерживается TOKEN для обратной совместимости.
TOKEN = os.getenv("BOT_TOKEN", "").strip() or os.getenv("TOKEN", "").strip()
...
async def main() -> None:
    if not TOKEN:
        raise RuntimeError("BOT_TOKEN не задан. Добавьте переменную окружения BOT_TOKEN в bothost.")
